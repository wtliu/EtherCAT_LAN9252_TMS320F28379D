...
UINT16 CiA402_Init(void)
{
    ...
    {
        /*Reset Axis buffer*/
//xxx 进行数据清除时memset函数中数量应该是对应字节数量的一半
        HMEMSET(&LocalAxes[AxisCnt],0,SIZEOF(TCiA402Axis)/2);
        ...
        *************************************/
//xxx 动态内存开辟时，应该是对应字节数的一半
        LocalAxes[AxisCnt].ObjDic = (TOBJECT *) ALLOCMEM(SIZEOF(DefCiA402AxisObjDic)/2);
...
/**
 ...
 \brief this functions provides an simple feedback functionality 此功能提供简单的反馈功能
*////////////////////////////////////////////////////////////////////////////////////////
void CiA402_DummyMotionControl(TCiA402Axis *pCiA402Axis)
{
    ...
    /*仅当应用程序由DC同步信号触发时，才会触发运动控制器，并设置了有效的操作模式*/

    /*calculate actual position 由实际速度计算实际位置 */
...
/**
 ...
 \CiA402-应用
 \检查状态转换是否未决并将所需的斜坡代码传递给CiA402TransitionAction（）
 \如果此函数返回true，则状态转换结束。
*////////////////////////////////////////////////////////////////////////////////////////
...
\brief      This function will copies the inputs from the local memory to the ESC memory
            to the hardware
*////////////////////////////////////////////////////////////////////////////////////////
//xxx 数据写入时，不能直接转换成对应结构体，将导致数据错位
        ...
        case 0:    //copy csp/csv TxPDO entries
            {
                *pTmpData = SWAPWORD(LocalAxes[AxisIndex].Objects.objStatusWord);
                pTmpData++;
                *pTmpData = SWAPDWORD(LocalAxes[AxisIndex].Objects.objPositionActualValue);
                pTmpData++;
                *pTmpData = SWAPDWORD(LocalAxes[AxisIndex].Objects.objPositionActualValue>>16);
                pTmpData++;
                *pTmpData = SWAPDWORD(LocalAxes[AxisIndex].Objects.objVelocityActualValue);
                pTmpData++;
                *pTmpData = SWAPDWORD(LocalAxes[AxisIndex].Objects.objVelocityActualValue>>16);
                pTmpData++;
                *pTmpData = SWAPWORD((LocalAxes[AxisIndex].Objects.objModesOfOperationDisplay & 0x00FF));
                pTmpData++;
            }
            break;
...
void APPL_OutputMapping(UINT16 *pData)
{
    ...
    for (j = 0; j < sRxPDOassign.u16SubIndex0; j++)
    {
        ...
        {
        case 0:    //csp/csv RxPDO    entries  速度位置模式
            {
            if(j == 1)
            {
                pTmpData++;
            ...
            pTmpData++;
            LocalAxes[AxisIndex].Objects.objTargetPosition |= (UINT32)SWAPWORD(*pTmpData) << 16;
            pTmpData++;
            LocalAxes[AxisIndex].Objects.objTargetVelocity  = SWAPWORD(*pTmpData);
            pTmpData++;
            LocalAxes[AxisIndex].Objects.objTargetVelocity |= (UINT32)SWAPWORD(*pTmpData) << 16;
            pTmpData++;
            LocalAxes[AxisIndex].Objects.objModesOfOperation = SWAPWORD(*pTmpData);
            pTmpData++;
            }
            break;
        case 1:    //csp RxPDO    entries  位置模式
            {
            LocalAxes[AxisIndex].Objects.objControlWord = SWAPWORD(*pTmpData);
            pTmpData++;
            LocalAxes[AxisIndex].Objects.objTargetPosition = SWAPWORD(*pTmpData);
...
\brief    This function will called from the synchronisation ISR 
            or from the mainloop if no synchronisation is supported
                            此函数将从同步ISR调用如果不支持同步，则从mainloop
*////////////////////////////////////////////////////////////////////////////////////////
