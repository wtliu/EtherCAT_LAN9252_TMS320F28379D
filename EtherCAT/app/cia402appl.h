...
-    StatusWord  状态
-----------------------------------------------*/
#define STATUSWORD_STATE_NOTREADYTOSWITCHON                  0x0000 /**< \brief Not ready to switch on还未准备好开机*/
...
/*---------------------------------------------
-    CiA402 State machine  状态机
-----------------------------------------------*/
#define STATE_NOT_READY_TO_SWITCH_ON        0x0001 /**< \brief Not ready to switch on (optional)尚未准备好开启（可选）*/
#define STATE_SWITCH_ON_DISABLED            0x0002 /**< \brief Switch on but disabled (optional)打开但禁用（可选）*/
#define STATE_READY_TO_SWITCH_ON            0x0004 /**< \brief Ready to switch on (mandatory)准备开启（强制）*/
#define STATE_SWITCHED_ON                   0x0008 /**< \brief Switch on (mandatory)打开（强制）*/
#define STATE_OPERATION_ENABLED             0x0010 /**< \brief Operation enabled (mandatory) 启用操作（强制）*/
#define STATE_QUICK_STOP_ACTIVE             0x0020 /**< \brief Quick stop active (optional)快速停止激活（可选）*/
#define STATE_FAULT_REACTION_ACTIVE         0x0040 /**< \brief Fault reaction active (mandatory)故障反应有效（强制）*/
#define STATE_FAULT                         0x0080 /**< \brief Fault state (mandatory)故障状态（强制）*/


/*---------------------------------------------
-    CiA402 Modes of Operation 运作模式 (object 0x6060) (IEC61800_184e)
-----------------------------------------------*/
// -128 to -1 Manufacturer-specific operation modes  制造商特定的操作模式
#define NO_MODE                     0 /**< \brief No mode*/
#define PROFILE_POSITION_MODE       1 /**< \brief Position Profile mode位置配置模式*/
#define VELOCITY_MODE               2 /**< \brief Velocity mode速度模式*/
#define PROFILE_VELOCITY_MOCE       3 /**< \brief Velocity Profile mode速度曲线模式*/
#define PROFILE_TORQUE_MODE         4 /**< \brief Torque Profile mode 扭矩曲线模式*/
//5 reserved                          
#define HOMING_MODE                 6 /**< \brief Homing mode*/
#define INTERPOLATION_POSITION_MODE 7 /**< \brief Interpolation Position mode 插值位置模式*/
#define CYCLIC_SYNC_POSITION_MODE   8 /**< \brief Cyclic Synchronous Position mode循环同步位置模式*/
#define CYCLIC_SYNC_VELOCITY_MODE   9 /**< \brief Cyclic Synchronous Velocity mode循环同步速度模式*/
#define CYCLIC_SYNC_TORQUE_MODE     10/**< \brief Cyclic Synchronous Torque mode循环同步转矩模式*/
//+11 to +127 reserved


/***************************************
 CiA402 Error Codes 错误代码 (object 0x603F) (IEC61800_184e)
 ***************************************/
#define ERROR_SHORT_CIRCUIT_EARTH_LEAKAGE_INPUT             0x2110 /**< \brief Short circuit/earth leakage (input)短路/漏电（输入）*/
...
#define MAX_AXES    2 /**< \brief Number of supported axes支持的轴数量*/



/*---------------------------------------------
-    Module Identifications 模块标识 (each supported operation mode has a module)
-----------------------------------------------*/
...
/**
 * \addtogroup PdoMappingObjects PDO Mapping Objects 添加到组PDO映射对象
...
typedef struct
{
    BOOL        bAxisIsActive; /**< \brief Indicates if active is active 表示当前是否处于活动状态*/
    BOOL        bBrakeApplied; /**< \brief Motor break is applied 应用电机断开*/
    BOOL        bLowLevelPowerApplied; /**< \brief Low Level Power applied低电源应用*/
    BOOL        bHighLevelPowerApplied; /**< \brief High Level Power applied高电源应用*/
    BOOL        bAxisFunctionEnabled; /**< \brief Axis functions enabled 轴功能已启用*/
    BOOL        bConfigurationAllowed; /**< \brief Configuration allowed 允许配置*/

    UINT16      i16State; /**< \brief Axis state轴状态*/
    UINT16      u16PendingOptionCode; /**< \brief Pending operation code待处理的操作代码*/
    double      fCurPosition; /**< \brief Current position within control loop控制回路中的当前位置*/
    UINT32      u32CycleTime; /**< \brief Motion controller cycletime in us运动控制器循环时间*/
